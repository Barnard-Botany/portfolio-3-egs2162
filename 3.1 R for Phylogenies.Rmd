...
install.packages("devtools") # have to download then load to use 
devtools::install_github("ropensci/brranching") # using devtool to install this from r repostory
library("ape")
library("brranching")
```
...
# plots appear identical when plotted
# but phylo with more specifics // plot more generic (just plug in x,y)
# diff in arguments to put in 
```
...
tr_primates 
# describes what it is (phyogenetic tree) and how many/type of each notde
# gives the laels of each tip - that is each of the species listed in the tree

str("tr_primates")
# outupts the name of the list/file (?): chr "tr_primates" 

str(tr_primates)
# it seems this gives data on the eduges - where/how the branching is structured
# too it tells how many (internal) nodes there are
# to it tells the names of the tips (species in tree)
# it gives 2 attributes (class and order) 

attributes(tr_primates)
# gives the name, class and order of the file - in a way easier to read than the str()
# all the attributes of the file (describes organization and type info it contains)

names(tr_primates) 
# lists the "edge", "Nnode", and "tip.label"
# edge may describe the number of branches? (including those that are internal) - links between nodes
...
```{r}
newick_str = "(((House Cat, Lion), House Dog), Elephant);"
tr_new = read.tree(text=newick_str)
 
plot.phylo(tr_new)

## Mini-Excercise 1
emily_str = "((Caterpillar, Snake),(Rhino, Mouse), Orangutang);"
tr2 = read.tree(text=emily_str)
...
Phylogenies have tips, nodes and edges. Nodes are ancestors to tips. Tips and edges are connected by nodes. Also, reecent or derived nodes can  connect to older or ancestral nodes. The details of how tips and nodes connect (via edges) is dictate by the newick code. 
...
# yes - a good visual represetnation, however I dont mind the Newick format for long lists either
...
# you can see the different categorizations by just looking at the code, but it doesnt indicate which is better in a sense of accuaty phylogenteic categorization - that I'll have to look up online (yes its better than the first - cabage and kale are closely related)
...
## (1)
newick_str = "((Escholzia californica, Lupinus bicolor),(Alra caryophyll,(Vromus diandrus, Bromus tectorum)));"
tr2_plants_new = read.tree(text=newick_str)
plot(tr2_plants_new)


## (2)
# A)
newick_str_correct = "((((Human, Mouse), Housefly), Radish, Sequoia), Gut Bacteria);"
newick_str_erroneous = "((((Human, Gut Bacteria), Radish, Horsefly), Mouse), Sequoia);" 
# problem wiht this tree: places the organisms way out of order (human and horsefly are the furthest apart - not closest); in the correct tree, they're categorized/grouped based on how similar they are (radish/sequoia placement may be lumped wrong)

tr_new_correct = read.tree(text=newick_str_correct)
tr_new_erroneous = read.tree(text=newick_str_erroneous)

plot.phylo(tr_new_correct)
plot.phylo(tr_new_erroneous)

# B) 
str(tr_new_correct)
str(tr_new_erroneous)
# differ in edges (1:10 vs 1:9), number of nodes (5 vs 4)
# diff in edges and nodes likely due to the fact that I split sequoia and radish in the same line for the "correct" tree but not in the "erroneous"
# the labels are in differnt orders (what I would hav assumed considering the two trees group organisms differently)
...
newick_str = "(((Humans:12.5, Chimps:12.5):1.0, Gorillas:14.0):1.0, (Orangs:14.5, Orangs2)):1.0;"
tr_primate = read.tree(text=newick_str)
plot(tr_primate)

## added () and "Orangs2" into the code to group Orangs and Organgs2
```
...
newick_str = "(((cabage, kale), radish), (corn, wheat), sequoia);"
tr_plants = read.tree(text=newick_str)
plot(tr_plants)
...
# right and left ladderalized in a matrix
layout(matrix(1:2, 1, 2))
plot(ladderize(tr_plants, FALSE), main = "left-ladderized and downwards", direction = c("downwards"))
plot(ladderize(tr_plants, FALSE), main = "right-ladderized and upwards", direction = c("upwards"))
...
# number is default (rep of a stored tree that we are referencing)
# get = 'POST' to look up and resolve each species and refrence to stored tree and then put into newick string
...
# EG1 and EG2 processed against diff trees 
```
...
```{r}
# 1. plotteing as phylograms and radial charts
layout(matrix(1:4, 2, 2))
plot(tree_phylomatic_EG1, no.margin = TRUE, type="phylogram", main = "EG1 phylogram")
plot(tree_phylomatic_EG2, no.margin = TRUE, type="phylogram", main = "EG2 phylogram")
plot(tree_phylomatic_EG1, no.margin = TRUE, type="radial", main = "EG1 radial")
plot(tree_phylomatic_EG2, no.margin = TRUE, type="radial", main = "EG2 radial")

# 2. The generated trees were hard to read due to the compactness of the labels. However, a clear differnce between the two trees was the length of the branches: EG1 was uniform, whereas EG2 variable and specfic. EG1 was newick for branching order, whereas EG2 included time. This difference in information altered the branch lengths in EG2 to represent the ages of the lineages and nodes. 

# 3. Further, there were some species exluded from the trees (26 for EG1 and 30 for EG2). It seems that the species left out were not angiosperms and thus not recognized by the database (which only holds data for angiosperms). As a result, there was nothing in the stored tree dataset by which it could be compared. It seems that the output, often "NA/genus/genus" indicates that there is somethign missung from the first data set - as seen in the "NA". However, there are a few insances in EG2 that dont feature an "NA": "pinaceae/abies/abies"; "ceratophyllaceae/ceratophyllum/ceratophyllum". Still, however they were unmatched. In the history of botany, this reveals the emphasis/bias placed on the study of angiosperms wihtin the realm of all plant studies. 
...
# made plant list and read using "read.csv" command
PlantList = read.csv("PlantList.txt")

EG<-c(PlantList)

# get tree from Zanne2014
tree_phylomatic <- phylomatic(taxa = EG, storedtree ="zanne2014", get = 'POST')

# plot
plot(tree_phylomatic, no.margin = TRUE, type="cladogram")
plot(tree_phylomatic, no.margin = TRUE, type="fan")
plot(tree_phylomatic, no.margin = TRUE, type="phylogram")
plot(tree_phylomatic, no.margin = TRUE, type="unrooted")
# ** for some reason, the console reads that 39 taxa aren't matched (indicating that the species from my list arent in the Sanne2014 database), but the tree is still generated


# save newick file
newick_emily = "tree_phylomatic_EMILY"
write.tree(tree_phylomatic, file=newick_emily)
```
