...
## to construct:
## 1. make taxa list using "<-c()"
## 2. phylomatic command to compile tree (here, can choose what tree its compared to)
## 3. draw/plot using "plot" command

## different trees for comparisom produce 2 very different tree outputs 
...
# SAME: 
# - the first separation groups are the same (neomarica, aechmea, saccharum, and amorpophallus) versus the rest
# - grouping of (neomarica, aechmea, saccharum, and amorpophallus) in both trees

# DIFF: branch length is diffeernt between the two trees as well 
# - orering of ends (one chart orders them temporally in a vertical way, and the other doesnt)
# - lenths of the branches (inclusion of timing of diverbende (horizontal time component))
...
## produces tree with 4 animals - check  
...
## line 313 having trouble running ? tr2_plants not found - never defined bu not sure whre is should be in the code 
```
...
```{r}
library(rotl)
taxa <- c("Abies", "Ailanthus", "Begonia", "Berberis", "Betula", "Chenopodium", "Clematis", "Dahlia", "Dracaena", "Equisetum", "Euonymus", "Euphorbia", "Fagus")
resolved_names <- tnrs_match_names(taxa)
resolved_names
...
```{r}
# tree for the firts 10 plants from my list of "daily plants" in portfolio 2 

Ptaxa <- c("Musa acuminata",
"Malus pumila",
"Oryza sativa",
"Camellia sinesnsis",
"Zea mays",
...
"Stevia rebaudiana",
"Vanilla planifolia")
P_resolved_names <- tnrs_match_names(Ptaxa)
in_tree <- is_in_tree(ott_id(P_resolved_names))
in_tree
